...
def get_user_settings():
    """
    Get the current user's cost filter settings.
    
    Returns:
        JSON object containing the current values of max_input_cost_filter and max_output_cost_filter.
    """
    try:
        return jsonify({
            'max_input_cost': current_user.max_input_cost_filter,
            'max_output_cost': current_user.max_output_cost_filter
        })
    except Exception as e:
...
def save_user_settings():
    """
    Save the user's cost filter settings.
    
    Expected payload:
        {
            'max_input_cost': <float or null>,
            'max_output_cost': <float or null>
        }
    
    Returns:
        JSON success response with a status message.
    """
    try:
        data = request.get_json()
        if not data:
        ...
        # Process max_input_cost
        if 'max_input_cost' in data:
            # Allow null values to clear the filter
            if data['max_input_cost'] is None:
                current_user.max_input_cost_filter = None
            else:
                # Validate as float
                try:
                    input_cost = float(data['max_input_cost'])
                    current_user.max_input_cost_filter = input_cost
                except ValueError:
                    return jsonify({
                        "success": False,
                        "error": "max_input_cost must be a valid number or null"
                    }), 400
        
        # Process max_output_cost
        if 'max_output_cost' in data:
            # Allow null values to clear the filter
            if data['max_output_cost'] is None:
                current_user.max_output_cost_filter = None
            else:
                # Validate as float
                try:
                    output_cost = float(data['max_output_cost'])
                    current_user.max_output_cost_filter = output_cost
                except ValueError:
                    return jsonify({
                        "success": False,
                        "error": "max_output_cost must be a valid number or null"
                    }), 400
