"""
Test script to isolate Azure Blob Storage initialization issue with Gevent monkey patching.
This script attempts to initialize the Azure BlobServiceClient with Gevent monkey patching.
"""
# Import and apply Gevent monkey patching at the very top
...
def test_azure_init_with_gevent():
    """
    Test Azure Blob Storage initialization with Gevent monkey patching.
    """
    try:
        ...
        logger.info("Initializing BlobServiceClient (with Gevent)...")
        ...
        logger.info("Azure Blob Storage initialization successful (with Gevent)")
        return True
    except Exception as e:
        logger.exception(f"Failed to initialize Azure Blob Storage (with Gevent): {e}")
        return False

if __name__ == "__main__":
    logger.info("Testing Azure Blob Storage initialization with Gevent monkey patching...")
    success = test_azure_init_with_gevent()
    if success:
        print("\n✅ Azure Blob Storage initialized successfully with Gevent")
    else:
        print("\n❌ Azure Blob Storage initialization failed with Gevent")