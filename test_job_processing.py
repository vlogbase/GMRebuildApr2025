...
# Example job functions that our worker will process
...
def example_report_job(user_id, report_type):
    ...
    return {"report_id": str(uuid.uuid4()), "user_id": user_id, "type": report_type}

def example_processing_job(file_path, user_id):
    """Example file processing job"""
    logger.info(f"Processing file {file_path} for user {user_id}")
    # In a real app, this would process a file
    time.sleep(1.5)  # Simulate work
    return {"status": "completed", "file": file_path, "user_id": user_id}

def example_cache_update_job(user_id):
    """Example cache update job"""
    logger.info(f"Updating cache for user {user_id}")
    # In a real app, this would update a cache
    time.sleep(0.5)  # Simulate work
    return {"cache_updated": True, "user_id": user_id, "timestamp": datetime.datetime.now().isoformat()}

def example_long_task(duration=5):
    """Example long-running task with progress reporting"""
    logger.info(f"Starting long task with duration {duration}s")
    
    # Simulate work with progress updates
    for i in range(duration):
        logger.info(f"Long task progress: {(i+1)/duration*100:.0f}%")
        time.sleep(1)
    
    return {"task_completed": True, "duration": duration}
